...
export const proxiedModels = allProviderModelData()
  .filter(model => !model.hideModel)
  .map(model => ({
    name: model.name,
...
These models are included in all <SubdomainLink href="/billing/">subscriptions</SubdomainLink>.
Per-token pricing is also available with <SubdomainLink href="/pricing">usage-based billing</SubdomainLink>.

<Alert className="mb-4">
  <InfoIcon color="#00bb00" />
  <AlertTitle>Model Details</AlertTitle>
  <AlertDescription>
    <p>More information about each model is available via the `/openai/v1/models` endpoint.</p>
  </AlertDescription>
</Alert>
