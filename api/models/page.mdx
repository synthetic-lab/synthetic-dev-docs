...
These models are included in all Standard and Pro <Anchor href={`${APP_BASE_URL}billing`}>subscriptions</Anchor>.
Per-token pricing is also available with [usage-based billing](/usage).
...
For GPU pricing details, see our <Anchor href={`${APP_BASE_URL}pricing#on-demand`}>pricing</Anchor> page.
...
Need help choosing the right model? Join our <Anchor href={`${APP_BASE_URL}discord`}>Discord community</Anchor> for recommendations!